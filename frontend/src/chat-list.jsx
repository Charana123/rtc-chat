...
function ChatCard(props){
    // Make REST API call to obtain available chats - their poster image, name and latest message
    return (
            ...
            <div className="container-fluid">
                <div className="row align-items-center">
                    <div className="col-auto px-2">
                        <button className="fa fa-plus fa-2x" data-toggle="modal" data-target="#NewChannelModal"/>
                    </div>
                    <div className="col pr-2">
                        <SearchBar 
                            onChange={props.onChangeChannelFilter}
                            onKey={props.onKeyChannelFilter}
                        />
                    </div>
                </div>
            </div>
...
export default function ChatList(props){
    return (
        <React.Fragment>
            <ChatBar 
                onChangeChannelFilter={props.onChangeChannelFilter}
                onKeyChannelFilter={props.onKeyChannelFilter}
            />
            {
                props.channels.map((channel) => {
                    return <ChatCard key={channel} channel={channel}/>
                })
            }
        </React.Fragment>
    )
